...
def obter_dados_gex(ticker_symbol, taxa_juros=0.045, multiplicador_carrego=1.0):
    ...
    for _, row in calls.iterrows():
        ...
        if vol > 0 and oi > 0:
            ...
            gex_data.append({'strike': K, 'gex': gex, 'type': 'call'})
    ...
    for _, row in puts.iterrows():
        ...
        if vol > 0 and oi > 0:
            gamma = black_scholes_gamma(spot_price, K, T, taxa_juros, vol)
            gex = -(gamma * oi * 100 * spot_price * 0.01)
            gex_data.append({'strike': K, 'gex': gex, 'type': 'put'})
    ...
    # Mapeamento dos Níveis Operacionais
    ...
    flip_idx = (np.diff(np.sign(df_gex['cumsum'])) != 0).argmax()
    gamma_flip = df_gex.iloc[flip_idx]['strike']
